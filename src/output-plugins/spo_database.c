			      ...
			      u_int32_t i_sig_id)
{
    ...
    */
    ...
    if(event != NULL)
    {
			buf = GetTimestampByComponent_STATIC(
    ...
    {
			buf = GetTimestampByStruct_STATIC((struct timeval *) &p->pkth->ts, data->tz,data->timestampHolder));
			if(buf)
					 {
	    	 /* XXX */
				 free(buf);
				...
				buf = GetCurrentTimestamp_STATIC(data->timestampHolder);
			if(buf)
			{
	    /* XXX */
				free(buf);
